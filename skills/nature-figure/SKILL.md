...
## Personal figure style memory

When the user asks to use their personal plotting style, learn from their figure revisions, or synchronize skills, read:

```text
C:\Users\dess\.codex\research-memory\figure-style-profile.yaml
```

Use it as a preference layer after the figure contract and backend gate. It can guide font sizes, font family, palettes, line widths, markers, legends, colorbars, panel labels, and export settings. Do not update it silently. If the user asks you to learn from a modified plot or script, first produce a concise proposal of style updates and wait for approval before writing memory.
